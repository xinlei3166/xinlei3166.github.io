...
<span class="line"><span></span></span>
<span class="line"><span>当然也可以下载源码进行安装，比如：</span></span></code></pre></div><p>wget <a href="https://pypi.python.org/packages/source/s/supervisor/supervisor-3.1.3.tar.gz" target="_blank" rel="noreferrer">https://pypi.python.org/packages/source/s/supervisor/supervisor-3.1.3.tar.gz</a> --no-check-certificat</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>tar -zxvf supervisor-3.1.3.tar.gz</span></span>
<span class="line"><span>cd supervisor-3.1.3</span></span>
<span class="line"><span>sudo python setup.py install</span></span></code></pre></div><p>安装之后可以直接supervisord运行验证是否成功，如果报错，再逐一解决，比如可能会报meld3版本问题，这里给出安装步骤：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>wget http://effbot.org/media/downloads/elementtree-1.2.7-20070827-preview.zip</span></span>
<span class="line"><span>unzip elementtree-1.2.7-20070827-preview.zip  &amp;&amp;  cd elementtree-1.2.7-20070827-preview</span></span>
<span class="line"><span>python setup.py install</span></span></code></pre></div><p>或者下载此版本：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>wget http://www.plope.com/software/meld3/meld3-0.6.5.tar.gz</span></span>
<span class="line"><span>tar -xf meld3-0.6.5.tar.gz &amp;&amp; cd meld3-0.6.5</span></span>
<span class="line"><span>python setup.py install</span></span></code></pre></div><p>如果安装成功就可以进行下一步了：设置配置文件。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>### 生成配置文件，且放在/etc目录下</span></span>
...
<span class="line"><span>password=123</span></span></code></pre></div><p>启动supervisord</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span> # supervisord -c /etc/supervisord.conf</span></span></code></pre></div><p>查看一下是否监听</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span> # lsof -i:9001</span></span>
<span class="line"><span> COMMAND     PID USER   FD   TYPE   DEVICE SIZE/OFF NODE NAME</span></span>
<span class="line"><span> superviso 14685 root    4u  IPv4 20155719      0t0  TCP *:etlservicemgr (LISTEN)</span></span></code></pre></div><p>现在通过 <a href="http://ip:9001/" target="_blank" rel="noreferrer">http://ip:9001/</a> 就可以查看supervisor的web界面了(默认用户名及密码是user和123)，当然目前还没有加入任何监控的程序。</p><p><img src="http://i.imgur.com/XnqmIH5.png" alt="img"></p><p>下面写一个简单的python脚本，用来验证supervisor的监控效果。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>#cat /root/temp/test_http.py   ###以下即是test_http.py脚本中的代码</span></span>
...
<span class="line"><span>    httpd.serve_forever()</span></span></code></pre></div><p>增加一个配置文件，以便supervisor用来监控test_http.py程序。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>#cat /etc/supervisord.d/supervisor_test_http.conf  ### 以下即为配置文件中的内容</span></span>
...
<span class="line"><span>- password：认证密码</span></span></code></pre></div><ul><li>inet<em>http</em>server配置块</li></ul><p>在该配置块的参数项表示的是一个监听在TCP上的HTTP server，如果[inet<em>http</em>server]块不在配置文件中或被注释，则不会启动基于TCP的HTTP server。该块的参数介绍如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>- port：TCP监听的地址和端口(ip:port)，这个地址会被HTTP/XML-RPC监听</span></span>
<span class="line"><span>- username：HTTP server在认证时的用户名</span></span>
<span class="line"><span>- password：认证密码</span></span></code></pre></div><p>比如：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span> [inet_http_server]         ; inet (TCP) server disabled by default</span></span>
...
<span class="line"><span> password=123               ; (default is no password (open server))</span></span></code></pre></div><p>表示监听在9001端口，需要使用用户名+密码的方式访问，访问地址是:http//127.0.0.1:9001。</p><ul><li>supervisord配置块</li></ul><p>该配置块的参数项是关于supervisord进程的全局配置项。该块的参数介绍如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>- logfile：log文件路径</span></span>
...
<span class="line"><span>- environment：一个k/v对的list列表</span></span></code></pre></div><p>该块的参数通常不需要改动就可以使用，当然也可以按需修改。</p><ul><li>program配置块</li></ul><p>该块就是我们要监控的程序的配置项。该配置块的头部是有固定格式的，一个关键字program，后面跟着一个冒号，接下来才是程序名。例如：[program:foo]，foo就是程序名，在使用supervisorctl来操作程序的时候，就是以foo来标明的。该块的参数介绍如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>- command：启动程序使用的命令，可以是绝对路径或者相对路径</span></span>
...
<span class="line"><span>- serverurl：是否允许子进程和内部的HTTP服务通讯，如果设置为AUTO，supervisor会自动的构造一个url</span></span></code></pre></div><p>比如下面这个选项块就表示监控一个名叫test_http的程序：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>[program:test_http]</span></span>
...
<span class="line"><span>cd cesi &amp;&amp; python web.py     ### 即可启动成功</span></span></code></pre></div><p>cesi.conf配置文件的设置：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>[node:local]                            ### 设置监控的各个机器</span></span>
<span class="line"><span>username = user</span></span>
...
<span class="line"><span>host = 0.0.0.0</span></span></code></pre></div><p>一切顺利的话，可通过页面<a href="http://ip:5000" target="_blank" rel="noreferrer">http://ip:5000</a>，用户名，密码都是admin，最终的效果如下所示： <img src="http://i.imgur.com/0UBx7qF.png" alt="img"></p><p>cesi repo上的示例效果：</p><p><img src="https://github.com/GulsahKose/cesi/raw/master/screenshots/image1" alt="img"></p><p>记录下用法学习使用。</p><p>转载自：<a href="https://www.cnblogs.com/smail-bao/p/5673434.html" target="_blank" rel="noreferrer">https://www.cnblogs.com/smail-bao/p/5673434.html</a></p>`,76),i=[t];function r(l,o,c,u,d,h){return a(),n("div",null,i)}const m=s(e,[["render",r]]);export{g as __pageData,m as default};
